...
///
/// ACP-platform short-circuit mirrors `display_for`: an ACP client's
/// `agent_message_chunk` stream is append-only, so a re-rendered tool-status
/// prefix would corrupt the deltas. Relay masking is a Slack/Discord-side
/// concern and does not apply there.
